...
{
    class s_Const_CuttingRecipe_TimeToCut : Sensor
    {
		private int counter;
		...
		public override void Initialize(SensorConfiguration sensorConfiguration)
		{
			...
			counter = 0;
			mappingTemplate = "s_Const_CuttingRecipe_TimeToCut(constantsSensor,objectIndex("+index+"),{1},{0})." + Environment.NewLine;
		...
		public override void Update()
		{
			...
			if(!invariant || first)
			{
				...
				for(int i_1 = 0; i_1 < CuttingRecipes1.Count; i_1++)
				{
					...
					int TimeToCut2 = CuttingRecipes1[i_1].TimeToCut;
					if (values[i_1].Count == 200)
					{
							values[i_1].RemoveAt(0);
					}
					values[i_1].Add(TimeToCut2);
				}
			}
		}

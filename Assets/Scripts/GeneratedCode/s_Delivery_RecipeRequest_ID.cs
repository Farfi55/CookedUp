...
{
    class s_Delivery_RecipeRequest_ID : Sensor
    {
		private int counter;
		...
		public override void Initialize(SensorConfiguration sensorConfiguration)
		{
			...
			counter = 0;
			mappingTemplate = "s_Delivery_RecipeRequest_ID(deliverySensor,objectIndex("+index+"),{1},{0})." + Environment.NewLine;
		...
		public override void Update()
		{
			...
			if(!invariant || first)
			{
				...
				for(int i_1 = 0; i_1 < WaitingRecipesRequests1.Count; i_1++)
				{
					...
					int ID2 = WaitingRecipesRequests1[i_1].ID;

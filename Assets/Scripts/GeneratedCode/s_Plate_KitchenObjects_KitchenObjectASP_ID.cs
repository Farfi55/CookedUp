...
{
    class s_Plate_KitchenObjects_KitchenObjectASP_ID : Sensor
    {
		private int counter;
		...
		public override void Initialize(SensorConfiguration sensorConfiguration)
		{
			...
			counter = 0;
			mappingTemplate = "s_Plate_KitchenObjects_KitchenObjectASP_ID(plateSensor,objectIndex("+index+"),{1},{0})." + Environment.NewLine;
		...
		public override void Update()
		{
			...
			if(!invariant || first)
			{
				...
				for(int i_1 = 0; i_1 < KitchenObjects1.Count; i_1++)
				{
					...
					int ID2 = KitchenObjects1[i_1].ID;

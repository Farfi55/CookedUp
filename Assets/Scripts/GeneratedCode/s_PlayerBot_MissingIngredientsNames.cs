...
{
    class s_PlayerBot_MissingIngredientsNames : Sensor
    {
		private int counter;
		...
		public override void Initialize(SensorConfiguration sensorConfiguration)
		{
			...
			counter = 0;
			mappingTemplate = "s_PlayerBot_MissingIngredientsNames(player,objectIndex("+index+"),{1},{0})." + Environment.NewLine;
		...
		public override void Update()
		{
			...
			if(!invariant || first)
			{
				...
				if(MissingIngredientsNames1 == null)
				{
					values.Clear();
					return;
				}
				else if(MissingIngredientsNames1.Count > values.Count)
				{
					for(int i = values.Count; i < MissingIngredientsNames1.Count; i++)
					{
				...
				{
					for(int i = MissingIngredientsNames1.Count; i < values.Count; i++)
					{
						values.RemoveAt(values.Count - 1);
					}
				}
				for(int i_1 = 0; i_1 < MissingIngredientsNames1.Count; i_1++)
				{
					string MissingIngredientsNames2 = MissingIngredientsNames1[i_1];
					if(MissingIngredientsNames2 == null)
					{
						values[i_1].Clear();
						continue;
					}
					else
					{
						...
						values[i_1].Add(MissingIngredientsNames2);
					}
				}
			}
		}

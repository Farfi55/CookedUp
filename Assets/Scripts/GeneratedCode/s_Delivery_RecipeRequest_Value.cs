...
{
    class s_Delivery_RecipeRequest_Value : Sensor
    {
		private int counter;
		...
		public override void Initialize(SensorConfiguration sensorConfiguration)
		{
			...
			counter = 0;
			mappingTemplate = "s_Delivery_RecipeRequest_Value(deliverySensor,objectIndex("+index+"),{1},{0})." + Environment.NewLine;
		...
		public override void Update()
		{
			...
			if(!invariant || first)
			{
				...
				for(int i_1 = 0; i_1 < WaitingRecipesRequests1.Count; i_1++)
				{
					...
					int Value2 = WaitingRecipesRequests1[i_1].Value;
					if (values[i_1].Count == 200)
					{
							values[i_1].RemoveAt(0);
					}
					values[i_1].Add(Value2);
				}
			}
		}

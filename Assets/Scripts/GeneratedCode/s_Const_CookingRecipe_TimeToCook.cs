...
{
    class s_Const_CookingRecipe_TimeToCook : Sensor
    {
		private int counter;
		...
		public override void Initialize(SensorConfiguration sensorConfiguration)
		{
			...
			counter = 0;
			mappingTemplate = "s_Const_CookingRecipe_TimeToCook(constantsSensor,objectIndex("+index+"),{1},{0})." + Environment.NewLine;
		...
		public override void Update()
		{
			...
			if(!invariant || first)
			{
				...
				for(int i_1 = 0; i_1 < CookingRecipes1.Count; i_1++)
				{
					...
					int TimeToCook2 = CookingRecipes1[i_1].TimeToCook;
					if (values[i_1].Count == 200)
					{
							values[i_1].RemoveAt(0);
					}
					values[i_1].Add(TimeToCook2);
				}
			}
		}
